...
import { TxSummary, AddressSummary } from './types';
...
export class CardanoExplorerAPI {
  ...
  async addressesSummary(address: string) {
    try {
      ...
      return Promise.resolve(response.data as AddressSummary);
    } catch (error) {
      return Promise.reject(error);
    }
  }

  async txSummary(txId: string) {
    try {
      if (!txId) throw new Error('Missing txId');
      const response = await this.axios.request({
        url: `/txs/summary/${txId}`,
      });
      return Promise.resolve(response.data as TxSummary);
    } catch (error) {
      return Promise.reject(error);
    }
  }
